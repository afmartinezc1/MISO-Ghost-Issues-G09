...
describe("change password Staff", () => {
  it("Como usuario inicio sesion en ghost, entro a staff, y modifico la contraseña de un usuario, me deslogueo e ingreso con la nueva", async () => {
    cy.pseudoData().then((response) => {
      ...
      cy.get(pageElements.login.userInput).type(pseudoData[0].email);
      //And I wait for 1 seconds
      cy.wait(1000);
      //And I enter password pseudoData[0].password
      cy.get(pageElements.login.passInput).type(pseudoData[0].password);
      //And I wait for 1 seconds
      cy.wait(1000);
      //And I click SignIn
      cy.get(pageElements.login.loginBtn).click();
