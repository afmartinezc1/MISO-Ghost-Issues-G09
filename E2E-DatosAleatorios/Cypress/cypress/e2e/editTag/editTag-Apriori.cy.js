...
describe("Edit Tag", () => {
  ...
  it("Como usuario inicio sesion en ghost, creo y selecciono un tag, edito su información y reviso que haya quedado en la lista de tags con los datos editados", async () => {
    cy.aprioriData().then((aprioriData) => {
        ...
        .type(`{selectall}${aprioriData[1].title_two}`);
        ...
        .type(`{selectall}${aprioriData[1].color.substring(1)}`);
        // And I enter tag description a-priori-1
        cy.get(pageElements.tag.inputDescription)
        .type(`{selectall}${aprioriData[1].text.substring(0,500)}`);
        // And I save tag changes
        cy.get(pageElements.tag.saveBtn).should('be.enabled').click();
        // And I wait for 3 seconds
        cy.wait(3000);
        ...
        // And I wait for 2 seconds
        cy.wait(2000);
        // Then I should see tag in tag list with name a-priori-1 and description a-priori-1
        cy.get(pageElements.tag.tagList)
        .should('contain', aprioriData[1].title_two)
        .and('contain', aprioriData[1].text.substring(0,500));
    });
  });
});
