...
describe("Edit Staff", () => {
  it("Como usuario inicio sesion en ghost, entro a staff, y modifico algunos de los atributos del usuario. Luego valido que estos atributos se hayan modificado correctamente", async () => {
    cy.pseudoData().then((response) => {
      ...
      cy.get(pageElements.staff.inputName).type(pseudoData[0].first_name);
      //And I clear user location in staf
      cy.get(pageElements.staff.inputLocation).invoke("val", "");
      //And I wait for 1 seconds
      cy.wait(1000);
      //And I fill user location in staff with text pseudoData[0].address
      cy.get(pageElements.staff.inputLocation).type(pseudoData[0].address);
      //And I clear user location in staf
      cy.get(pageElements.staff.inputBio).invoke("val", "");
      //And I wait for 1 seconds
      cy.wait(1000);
      //And I fill user biography in staff with text "<LOREM>"
      cy.get(pageElements.staff.inputBio).type(pseudoData[0].last_name);
      //And I wait for 1 seconds
      cy.wait(1000);
      ...
      cy.get(pageElements.staff.userEditedName).should("have.text", pseudoData[0].first_name);
      //And I wait for 2 seconds
      cy.wait(2000);
      //And I click in author user to modify
      cy.get(pageElements.staff.userToEdit).click();
      ...
      cy.get(pageElements.staff.inputName).should("have.value", pseudoData[0].first_name);
      //And I wait for 1 seconds
      cy.wait(1000);
      //And I should see user location in edit user with text pseudoData[0].address
      cy.get(pageElements.staff.inputLocation).should(
        "have.value",
        pseudoData[0].address
      );
      //And I wait for 1 seconds
      cy.wait(1000);
      //Then I should see user biography in edit user with text "<pseudoData[0].last_name>"
      cy.get(pageElements.staff.inputBio).should("have.value", pseudoData[0].last_name);
    });
  });
});
