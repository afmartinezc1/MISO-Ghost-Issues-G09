...
describe("Delete post draft", () => {
  it("Como usuario inicio sesion en ghost, creo un post, reviso que esté en la lista de draft posts, creo otro post, reviso que esté en la lista de draft posts, borro este post, y valido que el primer post de la lista de draft posts sea el primero que cree, con lo cual me cercioro que el segundo post efectivamente fue borrado", async () => {
    cy.aprioriData().then((aprioriData) => {
      ...
      // And I wait for 2 seconds
      cy.wait(2000);
      // Then I should see the first draft post with title apriori1
      ...
      // And I wait for 2 seconds
      cy.wait(2000);
      // Then I should see that the post content is apriori2
      ...
      // And I wait for 2 seconds
      cy.wait(2000);
      // Then I should see that the post title is apriori1
      cy.get(pageElements.post.postTitle).should(
        "have.value",
        aprioriData[0].title
      );
      // And I wait for 2 seconds
      cy.wait(2000);
      // Then I should see that the post content is apriori1
      cy.get(pageElements.post.postContent).should(
        "have.text",
        aprioriData[0].text
      );
