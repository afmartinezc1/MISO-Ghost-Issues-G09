Feature: change password Staff
  ...
  Scenario: Como usuario inicio sesion en ghost, entro a staff, y modifico la contraseña de un usuario, me deslogueo e ingreso con la nueva
    ...
    And I fill user email in staff with text "$email_1"
    And I wait for 1 seconds
    And I fill user new password in staff with text "$string_1"
    And I wait for 1 seconds
    And I fill user new password verification in staff with text "$$string_1"
    And I wait for 1 seconds
